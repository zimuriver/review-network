...
async function enrollRegistrar(orgName, orgMspId) {
    try {
        ...
        const caInfo = ccp.certificateAuthorities[`ca.${orgName}.review.com`];
        let caTLSCACerts;
        ...
        const walletPath = path.join(process.cwd(), `review_${orgName}_wallet`);
...
async function loadAdmin(orgName, orgMspId) {
    try {
        ...
        const keyPath = `../bash/crypto-config/peerOrganizations/${orgName}.review.com/users/admin@${orgName}.review.com/msp/keystore`;
	const keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
	const certPath = `../bash/crypto-config/peerOrganizations/${orgName}.review.com/users/admin@${orgName}.review.com/msp/signcerts`;
...
async function loadUser(orgName, orgMspId, userId) {
    try {
        ...
        const keyPath = `../bash/crypto-config/peerOrganizations/${orgName}.review.com/users/${userId}@${orgName}.review.com/msp/keystore`;
	const keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
	const certPath = `../bash/crypto-config/peerOrganizations/${orgName}.review.com/users/${userId}@${orgName}.review.com/msp/signcerts`;
	const certPEM = readAllFiles(certPath)[0];
