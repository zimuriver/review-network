...
setEnvironment() {
  ...
  CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/$ORG.review.com/peers/$PEER.$ORG.review.com/tls/ca.crt
  CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/$ORG.review.com/users/Admin@$ORG.review.com/msp
  CORE_PEER_ADDRESS=$PEER.$ORG.review.com:$PORT
  CORE_PEER_TLS_CERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/$ORG.review.com/peers/$PEER.$ORG.review.com/tls/server.crt
  CORE_PEER_TLS_KEY_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/$ORG.review.com/peers/$PEER.$ORG.review.com/tls/server.key
}

createChannel() {
  setEnvironment neuorg
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel create -o orderer.review.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/${CHANNEL_NAME}/channel.tx --connTimeout 120s >&log.txt
    res=$?
    set +x
  else
    set -x
    peer channel create -o orderer.review.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/${CHANNEL_NAME}/channel.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --connTimeout 120s >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
joinChannelWithRetry() {
  ...
  if [ $res -ne 0 -a $COUNTER -lt $MAX_RETRY ]; then
    ...
    echo "${PEER}.${ORG}.review.com failed to join the channel, Retry after $DELAY seconds"
    sleep $DELAY
    joinChannelWithRetry $ORG
  else
    COUNTER=1
  fi
  verifyResult $res "After $MAX_RETRY attempts, ${PEER}.${ORG}.review.com has failed to join channel '$CHANNEL_NAME' "
}
...
joinChannel() {
  ...
  for org in $ORG_LIST; do
    joinChannelWithRetry $org
    echo "===================== peer0.${org}.review.com joined channel '$CHANNEL_NAME' ===================== "
    sleep $DELAY
    echo
  done
}
...
joinNewPeerToChannel() {
  ...
  echo "===================== ${PEER}.${NEW_PEER_ORG}.review.com joined channel '$CHANNEL_NAME' ===================== "
}
...
updateAnchorPeersForOrg() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel update -o orderer.review.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/${CHANNEL_NAME}/${CORE_PEER_LOCALMSPID}anchors.tx --connTimeout 120s >&log.txt
    res=$?
    set +x
  else
    set -x
    peer channel update -o orderer.review.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/${CHANNEL_NAME}/${CORE_PEER_LOCALMSPID}anchors.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --connTimeout 120s >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
updateAnchorPeers() {
  ...
  for org in $ORG_LIST; do
    ...
    echo "===================== peer0.${org}.review.com set as anchor in ${org} in channel '$CHANNEL_NAME' ===================== "
    sleep $DELAY
    echo
  done
}
...
updateChannelConfiguration() {
  ...
  if [ "$NUM_ORGS_IN_CHANNEL" == "3" ]
  then
    ORG_LIST="neuorg conferenceorg"
  else
    ORG_LIST="neuorg thuorg pkuorg"
  fi
  for org in $ORG_LIST; do
    signConfigtxAsPeerOrg $org exportingEntityOrg_update_in_envelope.pb
    echo "===================== peer0.${org}.review.com signed update for channel '$CHANNEL_NAME' ===================== "
    echo
  done

  # Submit a channel configuration update transaction
  set -x
  peer channel update -f exportingEntityOrg_update_in_envelope.pb -c $CHANNEL_NAME -o orderer.review.com:7050 --tls --cafile $ORDERER_CA --connTimeout 120s >&log.txt
  res=$?
  set +x
  cat log.txt
...
updateAnchorPeerForNewOrg() {
  ...
  jq '.channel_group.groups.Application.groups.ExportingEntityOrg.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.exportingentityorg.review.com","port": 12051}]},"version": "0"}}' ${CHANNEL_NAME}_config.json > ${CHANNEL_NAME}_modified_config.json
  ...
  echo "===================== peer0.exportingentityorg.review.com signed update for channel '$CHANNEL_NAME' ===================== "
  echo

  # Submit a channel configuration update transaction
  set -x
  peer channel update -f exportingEntityOrg_anchor_update_in_envelope.pb -c $CHANNEL_NAME -o orderer.review.com:7050 --tls --cafile $ORDERER_CA --connTimeout 120s >&log.txt
  res=$?
  set +x
  cat log.txt
  verifyResult $res "Updating configuration for channel '"$CHANNEL_NAME"' failed"
