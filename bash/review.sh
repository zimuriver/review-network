...
function printHelp () {
  ...
  echo "      - 'startrest' - start all rest servers (thu, neu, conference)"
  echo "      - 'stoprest' - stop all rest servers (thu, neu, conference)"
  ...
  echo "    -g <org-name> - organization name: e.g., 'neuorg', 'thuorg'"
  ...
  echo
  echo "	./review.sh generate -c reviewchannel -o 3"
  echo "	./review.sh up -l logs/network.log"
  echo "	./review.sh down"
  echo "	./review.sh reset"
  echo "	./review.sh clean"
  echo "	./review.sh cleanall"
  echo
  echo "Taking all defaults:"
  echo "	review.sh generate"
  echo "	review.sh up"
  echo "	review.sh down"
}
...
function networkUp () {
  ...
  if [ ! -d "crypto-config" -o ! -f "channel-artifacts/genesis.block" -o ! -f "docker-compose-e2e.yaml" ]; then
    echo "Network artifacts or configuration missing. Run './review.sh generate -c <channel_name>' to recreate them."
    exit 1
  fi
...
function newPeerUp () {
  checkPrereqs
  # generate artifacts if they don't exist
  if [ ! -d "crypto-config/peerOrganizations/thuorg.review.com/peers/peer1.thuorg.review.com" ]; then
    echo "Peer artifacts or configuration missing. Run './review.sh createnewpeer' to recreate them."
    exit 1
  fi
  # Create folder for docker network logs
...
function newOrgNetworkUp () {
  checkPrereqs
  # generate artifacts if they don't exist
  if [ ! -d "crypto-config/peerOrganizations/exportingentityorg.review.com" ]; then
    echo "New org crypto artifacts missing. Run './review.sh createneworg -c <channel_name>' to create them."
    exit 1
  fi
  ...
  do
    ...
    then
      ...
      then
        echo "New org channel configuration missing in "$dir". Run './review.sh createneworg -c "$(basename $dir)"' to create it."
        exit 1
      fi
    fi
  done
...
function createChannel () {
  ...
  if [ ! -f "channel-artifacts/"${CHANNEL_NAME}"/channel.tx" ]; then
    ...
    echo "ERROR !!!! Run './review.sh generate -c "$CHANNEL_NAME"' to create this transaction file"
    exit 1
  fi
...
function joinNewPeerToChannel () {
  ...
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e PEER=peer1 -e ORG=thuorg $CONTAINER_CLI scripts/channel.sh joinnewpeer >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
...
function updateChannel () {
  ...
  if [ ! -f "channel-artifacts/"${CHANNEL_NAME}"/exportingEntityOrg.json" ]; then
    ...
    echo "ERROR !!!! Run './review.sh createneworg -c "$CHANNEL_NAME"' to create this transaction file"
    exit 1
  fi
...
function installContract () {
  ...
  # Package contract
  if [ "$CONTRACT_NAME" == "review" ]
  then
    CC_LANGUAGE=node
    PEERORGLIST="neuorg thuorg conferenceorg"
  elif [ "$CONTRACT_NAME" == "letterOfCredit" ]
  then
    CC_LANGUAGE=java
    PEERORGLIST="neuorg thuorg"
  elif [ "$CONTRACT_NAME" == "exportLicense" ]
  then
    CC_LANGUAGE=java
    PEERORGLIST="neuorg conferenceorg"
  elif [ "$CONTRACT_NAME" == "shipment" ]
  then
    CC_LANGUAGE=node
    PEERORGLIST="neuorg thuorg pkuorg"
  else
    echo "ERROR !!!! Unknown contract: "$CONTRACT_NAME
    exit 1
  fi
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e CC_LANGUAGE=$CC_LANGUAGE -e CC_LABEL=$CONTRACT_NAME -e CC_VERSION=v1 review_cli scripts/chaincode.sh package >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
  ...
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e PEERORGLIST="$PEERORGLIST" -e CC_LABEL=$CONTRACT_NAME -e CC_VERSION=v1 review_cli scripts/chaincode.sh install >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
  ...
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e NUM_ORGS_IN_CHANNEL=$NUM_ORGS_IN_CHANNEL -e PEERORGLIST="$PEERORGLIST" -e CC_LABEL=$CONTRACT_NAME -e CC_VERSION=v1 review_cli scripts/chaincode.sh approve >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
  ...
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e NUM_ORGS_IN_CHANNEL=$NUM_ORGS_IN_CHANNEL -e PEERORGLIST="$PEERORGLIST" -e CC_LABEL=$CONTRACT_NAME -e CC_VERSION=v1 review_cli scripts/chaincode.sh commit >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
...
function initContract () {
  ...
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e PEERORGLIST="$PEERORGLIST" -e CC_LABEL=$CONTRACT_NAME -e CC_FUNC=$CONTRACT_FUNC -e CC_ARGS="$CONTRACT_ARGS" review_cli scripts/chaincode.sh init >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
...
function upgradeContract () {
  ...
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e NUM_ORGS_IN_CHANNEL=$NUM_ORGS_IN_CHANNEL -e PEERORGLIST="$PEERORGLIST" -e CC_LANGUAGE=$CC_LANGUAGE -e CC_LABEL=$CONTRACT_NAME -e CC_VERSION=v2 -e OLD_CC_VERSION=v1 -e CC_FUNC=$CONTRACT_FUNC -e CC_ARGS="$CONTRACT_ARGS" review_cli scripts/chaincode.sh upgrade >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
...
function invokeContract () {
  ...
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e PEERORGLIST="$PEERORGLIST" -e CC_LABEL=$CONTRACT_NAME -e CC_FUNC=$CONTRACT_FUNC -e CC_ARGS="$CONTRACT_ARGS" -e ORGANIZATION=$ORGANIZATION review_cli scripts/chaincode.sh invoke >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
...
function queryContract () {
  ...
  docker exec -e CHANNEL_NAME=$CHANNEL_NAME -e CC_LABEL=$CONTRACT_NAME -e CC_FUNC=$CONTRACT_FUNC -e CC_ARGS="$CONTRACT_ARGS" -e ORGANIZATION=$ORGANIZATION review_cli scripts/chaincode.sh query >>$LOG_FILE_CLI 2>&1
  if [ $? -ne 0 ]; then
...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' orderer.review.com |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  then
    ...
    ORDERERS="orderer.review.com orderer2.review.com orderer3.review.com orderer4.review.com orderer5.review.com"
  else
    COMPOSE_FILES="-f $COMPOSE_FILE"
    ORDERERS="orderer.review.com"
  fi
  ...
  for PEER in peer0.neuorg.review.com peer0.thuorg.review.com peer0.pkuorg.review.com peer0.conferenceorg.review.com; do
  ...
  for CA in neu thu pku conference; do
    ...
    docker cp -a ca.${CA}org.review.com:/etc/hyperledger/fabric-ca-server $LEDGERS_BACKUP/${CA}-ca/
...
function networkDown () {
  ...
  for PEER in peer0.neuorg.review.com peer0.thuorg.review.com peer0.pkuorg.review.com peer0.conferenceorg.review.com peer1.thuorg.review.com peer0.exportingentityorg.review.com; do
    # Remove any old containers and images for this peer
...
function newPeerDown () {
  ...
  docker volume rm ${COMPOSE_PROJECT_NAME}_peer1.thuorg.review.com
}
...
function newOrgNetworkDown () {
  ...
  for PEER in peer0.exportingentityorg.review.com; do
    # Remove any old containers and images for this peer
    CC_CONTAINERS=$(docker ps -a | grep dev-$PEER | awk '{print $1}')
    if [ -n "$CC_CONTAINERS" ] ; then
      docker rm -f $CC_CONTAINERS
  ...
  docker volume rm ${COMPOSE_PROJECT_NAME}_peer0.exportingentityorg.review.com
}

# Delete dynamically created credentials, like wallet identities
function cleanDynamicIdentities () {
  # remove wallet identities
  rm -rf ../wallets/neuorg/*
  rm -rf ../wallets/thuorg/*
  rm -rf ../wallets/pkuorg/*
  rm -rf ../wallets/conferenceorg/*
...
function generateCerts () {
  ...
  if [ -d "crypto-config" ]; then
    echo "'crypto-config' folder already exists. Run 'rm -rf crypto-config' to delete existing credentials or './review.sh -cleanall' to delete all existing artifacts if you wish to start from a clean slate."
    return
  fi
  set -x
...
function generateCertsForRaftOrderingNodes () {
  ...
  if [ -d "crypto-config/ordererOrganizations/review.com/orderers/orderer2.review.com/" ]; then
    echo "'crypto-config' already contains credentials for multiple ordering nodes. Run 'rm -rf crypto-config' to delete existing credentials or './review.sh -cleanall' to delete all old artifacts if you wish to start from a clean slate."
    return
  fi
  set -x
...
function generateCertsForNewPeer () {
  ...
  if [ ! -d "crypto-config/peerOrganizations/thuorg.review.com" ]; then
    echo "No crypto artifacts found for thu org. Please generate that first before trying to add a new peer."
    exit 1
  fi
  set -x
  cryptogen extend --input=crypto-config --config=./add_peer_thu/crypto-config.yaml
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate certificates for new peer..."
...
function generateCertsForNewOrg () {
  ...
  if [ -d "crypto-config/peerOrganizations/exportingentityorg.review.com" ]; then
    echo "Crypto artifacts already exist for 'exportingentityorg.review.com'. Delete the folder 'crypto-config/peerOrganizations/exportingentityorg.review.com' and re-run this operation if you wish to generate fresh artifacts."
    return
  fi
  set -x
...
# (``peer0.neuorg.review.com`` & ``peer0.thuorg.review.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  if [ "$DEV_MODE" = true ] ; then
    PROFILE=OneOrgReviewOrdererGenesis
    CHANNEL_PROFILE=OneOrgreviewChannel
  elif [ "$ORDERER_MODE" = "prod" ]
  then
    PROFILE=reviewMultiNodeEtcdRaft
    if [ "$NUM_ORGS_IN_CHANNEL" == "3" ]
    then
      CHANNEL_PROFILE=TrackingChannel
    elif [ "$NUM_ORGS_IN_CHANNEL" == "4" ]
    then
      CHANNEL_PROFILE=ReviewChannel
    else
    ...
    PROFILE=BlockchainReviewOrdererGenesis
    if [ "$NUM_ORGS_IN_CHANNEL" == "3" ]
    then
      CHANNEL_PROFILE=TrackingChannel
    elif [ "$NUM_ORGS_IN_CHANNEL" == "4" ]
  ...
  if [ "$DEV_MODE" = false ] ; then
    echo
    echo "#####################################################################"
    echo "#######   Generating anchor peer update for NeuOrg    ##########"
    echo "#####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/${CHANNEL_NAME}/NeuOrgMSPanchors.tx -asOrg NeuOrg -channelID $CHANNEL_NAME
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for NeuOrg..."
      exit 1
    fi

    echo
    echo "#####################################################################"
    echo "#######   Generating anchor peer update for ThuOrg    ##########"
    echo "#####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/${CHANNEL_NAME}/ThuOrgMSPanchors.tx -asOrg ThuOrg -channelID $CHANNEL_NAME
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for ThuOrg..."
      exit 1
    fi
    ...
    then
      echo
      echo "####################################################################"
      echo "#######   Generating anchor peer update for ConferenceOrg    ##########"
      echo "####################################################################"
      set -x
      configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/${CHANNEL_NAME}/PkuOrgMSPanchors.tx -asOrg PkuOrg -channelID $CHANNEL_NAME
       configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/${CHANNEL_NAME}/ConferenceOrgMSPanchors.tx -asOrg ConferenceOrg -channelID $CHANNEL_NAME
      res=$?
      set +x
      if [ $res -ne 0 ]; then
        echo "Failed to generate anchor peer update for PkuOrg..."
        exit 1
      fi
    fi
    ...
    echo
    echo "######################################################################"
    echo "#######   Generating anchor peer update for PkuOrg    ##########"
    echo "######################################################################"
    set -x      
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/${CHANNEL_NAME}/PkuOrgMSPanchors.tx -asOrg PkuOrg -channelID $CHANNEL_NAME
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for ConferenceOrg..."
      exit 1
    fi
    echo
  fi
...
CHANNEL_NAME_DEV_MODE=review-dev-channel
COMPOSE_FILE=docker-compose-e2e.yaml
COMPOSE_FILE_NEW_PEER=docker-compose-another-thu-peer.yaml
